...
#include <stdio.h> 
#include <pthread.h> //edw

pthread_mutex_t mymutex1 = PTHREAD_MUTEX_INITIALIZER;             // edwwwww

DB* db_open_ex(const char* basedir, uint64_t cache_size)
{
    
    //pthread_mutex_lock(&mymutex1);                             // TWRA
    
    DB* self = calloc(1, sizeof(DB));

    //pthread_mutex_unlock(&mymutex1);                             //TWRA
...
DB* db_open(const char* basedir)
{
    ...
    DB *save_db_open_ex = db_open_ex(basedir, LRU_CACHE_SIZE);

    pthread_mutex_unlock(&mymutex1);                             //TWRA

    return save_db_open_ex;
}
...
int db_add(DB* self, Variant* key, Variant* value)
{
    ...
    int save_memtable_add = memtable_add(self->memtable, key, value);

    //pthread_mutex_unlock(&mymutex1);                             //TWRA
    return save_memtable_add;
}
...
int db_get(DB* self, Variant* key, Variant* value)
{
    ...
    if (memtable_get(self->memtable->list, key, value) == 1)   
        return 1;
    //pthread_mutex_unlock(&mymutex1);                             //TWRA
    int save_sst_get = sst_get(self->sst, key, value);
