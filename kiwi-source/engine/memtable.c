...
pthread_mutex_t mymutex3 = PTHREAD_MUTEX_INITIALIZER;             //edwwwww
...
static int _memtable_edit(MemTable* self, const Variant* key, const Variant* value, OPT opt)
{
    ...
    //pthread_mutex_unlock(&mymutex3);                             //TWRA
...
{
    pthread_mutex_lock(&mymutex3);                             //TWRA    
    
    int save_memtable_edit = _memtable_edit(self, key, value, ADD);
    
    pthread_mutex_unlock(&mymutex3);                             //TWRA
    return save_memtable_edit;
}
